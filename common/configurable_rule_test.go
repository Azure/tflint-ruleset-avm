...
func (r *emittingRule) Name() string              { return "legacy_rule" }
...
func TestConfigurableRuleSeverity(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			rule := common.NewConfigurableRule(
				"avm_test_rule",
				&emittingRule{severity: test.defaultSeverity},
			)
			config := ""
...
func TestConfigurableRuleRejectsInvalidSeverity(t *testing.T) {
	rule := common.NewConfigurableRule(
		"avm_test_rule",
		&emittingRule{severity: tflint.ERROR},
	)
